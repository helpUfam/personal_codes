...
public class CurlView extends GLSurfaceView implements View.OnTouchListener,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see br.org.sidi.coquetel.curl.CurlRenderer.Observer#onDrawFrame()
	 */
	@Override
	 ...
	 * 
	 * @see
	 * br.org.sidi.coquetel.curl.CurlRenderer.Observer#onPageSizeChanged(int,
	 * int)
	 */
	@Override
	public void onPageSizeChanged(final int width, final int height) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see br.org.sidi.coquetel.curl.CurlRenderer.Observer#onSurfaceCreated()
	 */
	@Override
	...
	/**
	 * Listen when pageCurl is touched
	 */
	@Override
	public boolean onTouch(final View view, final MotionEvent mEvt) {
	 ...
	 */
	public void setAllowLastPageCurl(final boolean allowLastPageCurl) {
		allowLastPgCurl = allowLastPageCurl;
	}
	...
	private void setCurlPos(final PointF curlPos, final PointF curlDir,
		...
		if (mCurlState == CURL_RIGHT
			...
			if (curlDir.y != 0) {
				final float diffX = curlPos.x - pageRect.left;
				final float leftY = curlPos.y + (diffX * curlDir.x / curlDir.y);
			...
			if (curlDir.y != 0) {
				final float diffX = curlPos.x - pageRect.right;
				final float rightY = curlPos.y
						+ (diffX * curlDir.x / curlDir.y);
		...
		final double dist = Math.sqrt(curlDir.x * curlDir.x + curlDir.y
				* curlDir.y);
		if (dist != 0) {
			curlDir.x /= dist;
			curlDir.y /= dist;
	 ...
	 */
	public void setEnableTouchPressure(final boolean enTouchPressure) {
		this.enTouchPressure = enTouchPressure;
	}
	 ...
	 */
	public void setRenderLeftPage(final boolean renderLeftPage) {
	 ...
	 */
	public void setSizeChangedObserver(final SizeChangedObserver observer) {
	 ...
	 */
	private void updateCurlPos(final PointerPosition pointerPos) {
		...
		if (mCurlState == CURL_RIGHT
			...
			final float dist = (float) Math.sqrt(mCurlDir.x * mCurlDir.x
					+ mCurlDir.y * mCurlDir.y);

			// Adjust curl radius so that if page is dragged far enough on
			// opposite side, radius gets closer to zero.
			final float pageWidth = mRenderer.getPageRect(
					CurlRenderer.PAGE_RIGHT).width();
			...
			if (dist >= curlLen) {
				final double translate = (dist - curlLen) / 2;
				if (mViewMode == SHOW_TWO_PAGES) {
					mCurlPos.x -= mCurlDir.x * translate / dist;
				} else {
					final float pageLeftX = mRenderer
				...
				final double angle = Math.PI * Math.sqrt(dist / curlLen);
				final double translate = radius * Math.sin(angle);
				mCurlPos.x += mCurlDir.x * translate / dist;
				mCurlPos.y += mCurlDir.y * translate / dist;
			}
		}
		// Otherwise we'll let curl follow pointer position.
			...
			final float pageRightX = mRenderer
					.getPageRect(CurlRenderer.PAGE_RIGHT).right;
